...
angular.module('app').controller('homeCtrl', function ($scope, $timeout) {
  ...
  $(document).ready(function () {
    ...
    createjs.Tween.get(txt1, { loop: true }).to({ alpha: 0 }, 1500).call(textChange).to({ alpha: 1 }, 1500);
    tweenDatShit = function tweenDatShit(reset) {
      // if(reset){
      //     createjs.Tween.removeTweens(chevron)
      // }
      createjs.Tween.get(chevron, { loop: true }).to({ y: canvas.height - 60 }, 900).to({ y: canvas.height - 40 }, 900, createjs.Ease.bounceOut).to({ y: canvas.height - 60 }, 900);
    };
  ...
  function textChange() {
    var namesNStuff = ['Web Developer', 'Angular', 'Sass', 'Movie Enthusiest', 'Rocket League', 'Husband', 'Brazil Traveler', 'Metal Gear Solid Fan', 'Father', 'Audio Book Listener'];
